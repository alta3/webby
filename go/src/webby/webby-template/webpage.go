...
//
// func getsummary() http.Handler {
//     return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
//
// 		data, err := ioutil.ReadFile("ansible.json")
// 		if err != nil {
// 			fmt.Print(err)
// 			}
//
...
//     // //Iterate over all courses, copy Id, Name, and any Tags
//     // for _, ThisCourse := range cs.Cc {
//     //    fmt.Printf("Menu Item = %s, %s, %s\n", ThisCourse.Id, ThisCourse.Name, ThisCourse.Labs)
//     //    if  ThisCourse.Id == ss {
//     //        mi.Id=ThisCourse.Id
//     //        mi.Name=ThisCourse.Name
//     //        mi.Tags=ThisCourse.Tags
...
//          }
	...
	http.Handle("/js/", http.StripPrefix("/js/", http.FileServer(http.Dir("deploy/js"))))
        ...
        http.Handle("/layerslider/", http.StripPrefix("/layerslider/", http.FileServer(http.Dir("deploy/layerslider"))))
				http.Handle("/json/", http.StripPrefix("/json/", http.FileServer(http.Dir("deploy/json"))))
	...
	//http.Handle("/tracker/", http.StripPrefix("/tracker/", cs.TrackerTemplate()))
        ...
        http.Handle("/api/v1/events/",http.StripPrefix("/api/v1/events/", events.getevents()))
				//http.Handle("/api/v1/course/coursejson/", http.StripPrefix("/api/v1/course/coursejson/", cs.getcoursejson()))

	// Stripe Chckout
	http.Handle("/checkout", Checkout())
//	http.Handle("/success", getsummary())
